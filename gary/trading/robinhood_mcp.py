...
Install in Cursor via the deeplink (config must include ``type: http``):
...
Then: Settings → Tools & MCPs → **Connect** (desktop OAuth). Cloud agents do
not inherit that session automatically.

Official equities tools (see Robinhood "Agentic Trading overview"):
...
# Official tool names from Robinhood Agentic Trading. Override via env if the
# live server renames anything after you confirm with GetMcpTools.
_DEFAULT_TOOLS = {
